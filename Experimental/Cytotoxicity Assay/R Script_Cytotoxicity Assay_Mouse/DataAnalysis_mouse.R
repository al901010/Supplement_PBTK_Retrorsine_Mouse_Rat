...
# pairs plot
MCMC_plot <- subset(out_MCMC, select = c(sd,IC50))
...
# xbreaks      <- seq(0,24,by = 6)
  ...
  plot2 <-
    ...
    scale_y_continuous(
      limits = ylimits,
      breaks = ybreaks)+
    geom_hline(yintercept=0.494, linetype="dashed", color = "black", size=1.5)+
    annotate("text", x = 1.5, y = 8, label = "10% DMSO",size=5, hjust=1, vjust=1,
             color="black")+
    annotate("text", x = Inf-1, y = Inf-1, label = "Mouse",size=10, hjust=1.1, 
             vjust=1.2)+
    annotation_logticks(sides="b")
      
plot2
ggsave("plot_mouse_sim.png", bg="white", width = 5, height = 5)
