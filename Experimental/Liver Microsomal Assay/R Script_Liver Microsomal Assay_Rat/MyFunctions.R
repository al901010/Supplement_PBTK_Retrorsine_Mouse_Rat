...
GetPred3 <- function(est,dataset,t) {
  ...
  new_rows_1$Species                <- "rat"
  new_rows_1$Time                   <- t
  new_rows_1$Concentration          <- "1"
  new_rows_1$ActualConcentration    <- unique(dataset$ActualConcentration[which(dataset$Species=="rat"&dataset$Concentration=="1")])
  ...
  new_rows_15$Species                <- "rat"
  new_rows_15$Time                   <- t
  new_rows_15$Concentration          <- "15"
  new_rows_15$ActualConcentration    <- unique(dataset$ActualConcentration[which(dataset$Species=="rat"&dataset$Concentration=="15")])
  ...
  new_rows_50$Species                <- "rat"
  new_rows_50$Time                   <- t
  new_rows_50$Concentration          <- "50"
  new_rows_50$ActualConcentration    <- unique(dataset$ActualConcentration[which(dataset$Species=="rat"&dataset$Concentration=="50")])
  ...
  new_rows_200$Species                <- "rat"
  new_rows_200$Time                   <- t
  new_rows_200$Concentration          <- "200"
  new_rows_200$ActualConcentration    <- unique(dataset$ActualConcentration[which(dataset$Species=="rat"&dataset$Concentration=="200")])
...
GetPred <- function(est,dataset,t) {
  ...
  dataset$Fit[which(dataset$Concentration==1 & dataset$Species=="rat")]  <- rep(TransData(out$S_1),each=2)
  dataset$Fit[which(dataset$Concentration==15 & dataset$Species=="rat")] <- rep(TransData(out$S_15),each=2)
  dataset$Fit[which(dataset$Concentration==50 & dataset$Species=="rat")] <- rep(TransData(out$S_50),each=2)
  dataset$Fit[which(dataset$Concentration==200 & dataset$Species=="rat")]<- rep(TransData(out$S_200),each=2)
...
GetMinus2LL <- function(est,dataset,t)  {
  ...
  #--- CALCULATE SSR ------------------------------------------------------------
  SSR_r <-  sum((dataset_pred$NormalizedConcentration[which(dataset_pred$Species=="rat")] - dataset_pred$Fit[which(dataset_pred$Species == "rat")])^2)
  ...
  # constant
  const_r <- length(dataset_pred$Time[which(dataset_pred$Species=="rat")]) * log(2*pi*est["sd"]^2)

  # minus 2 log likelihood
  minus2LL_r <- 1/est["sd"]^2 * SSR_r + const_r
