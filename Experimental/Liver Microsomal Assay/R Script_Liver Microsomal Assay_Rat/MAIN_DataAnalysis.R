...
dataset <- subset(dataset, dataset$Species == "rat")
...
#rat
dataset$ModelSD[which(dataset$Species == "rat")]   <- est_hat["sd"]
dataset$InhibConc[which(dataset$Species == "rat")]  <- est_hat["EC50"]
dataset$MaxDepletionRate[which(dataset$Species == "rat")] <- est_hat["Vmax"]
dataset$MichaelisMentenConstant[which(dataset$Species == "rat")]<- est_hat["Km"]
...
# sample_params_plot <- subset(sample_params, select = c(EC50,Vmax,Km))
# 
...
load(file = "dataset_ppd.rda")
 ...
 plot <-
   ...
   scale_y_log10(
       limits = ylimits)+
   annotate("text", x = Inf-1, y = Inf-1, label = "Rat  ",size=12, hjust=1.2, 
            vjust=1.4)+
   annotation_logticks(sides="l",short = unit(2.5,"mm"),mid = unit(5,"mm"))
 
 plot
 ggsave("plotlivermicrosomes_rat.png", bg="white", width = 9, height = 9)
