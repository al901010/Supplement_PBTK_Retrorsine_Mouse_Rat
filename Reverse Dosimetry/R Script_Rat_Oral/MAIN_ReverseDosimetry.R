...
plot_invitro
ggsave("plot_rat_invitro.png", bg="white", width = 5, height = 5)
...
runtable_reversedosimetry <- data.frame(DOSEORIGINAL,DOSEORIGINALUNIT,DOSE,
                                        DOSEUNIT,ADM,CMAXULIV,CMAXULIVUNIT)
...
#rat
runtable_reversedosimetry <- PredRxODE(speciestype="rat10weeks",
                                     t=seq(0,2,by=0.01),
...
plot_invivo <-
  ...
  geom_point(mapping=aes(color=BiologicalReplicate,fill=BiologicalReplicate),
             shape=21,size=6,stroke=1.5, alpha=0.4)+
  labs(x="RET dose (mg/kg bw)",y="Liver integrity (%)")+
  scale_color_manual(values = c("#009E73","#E69F00","#56B4E9"))+
  scale_fill_manual(values =  c("#009E73","#E69F00","#56B4E9"))+
  theme(
    plot.background   = element_rect(fill = "white"),
  ...
  annotate("text", x = Inf-1, y = Inf-1, label = "Rat",size=10, hjust=1.1, 
           vjust=1.2)+
  annotation_logticks(sides="b")+
  geom_segment(aes(x = 79.9, y = 95, xend = 79.9, yend = 0),size=1,
               arrow = arrow(),color="darkgray")+
  geom_hline(yintercept=95, linetype="dashed", color = "darkgray", size=1)+
  geom_segment(aes(x = 79.9, y = 95, xend = 104, yend = 95),size=6)
  
plot_invivo
ggsave("plot_rat_invivo.png", bg="white", width = 5.45, height = 5)
