...
GetParameterVector <- function (drug, 
  ...
  CL           <- c(CLcanef=CLcanef,CLactin=CLactin,CLactef=CLactef,
                    PSdiff=PSdiff,Vmaxliv=Vmaxliv, Km=Km)
  
  # FRACTIONS OF METABOLITE FORMATION #########################################
  fDHRGSH      <- DrugData$fDHRGSH # Fraction of CLmet for GSH conjugates
  fDHRPROT     <- DrugData$fDHRPROT # Fraction of CLmet for DNA adduct
  fDHRDNA      <- DrugData$fDHRDNA # Fraction of CLmet for protein adducts
  fsum         <- DrugData$fsum
  
    
  # DISAPPEARANCE RATES OF METABOLITES ########################################
  lambda1DHRGSH <- DrugData$lambda1DHRGSH  # Depletion rate constant of GSH conjugates [1/h]
  lambda1DHRPROT<- DrugData$lambda1DHRPROT # Depletion rate constant of protein adducts [1/h]
  lambda1DHRDNA <- DrugData$lambda1DHRDNA  # 1st Depletion rate constant of DNA adducts [1/h]
  lambda2DHRDNA <- DrugData$lambda2DHRDNA  # 2nd Depletion rate constant of DNA adducts [1/h]
  kDHRDNA       <- DrugData$kDHRDNA        # Transition rate constant of DNA adducts [1/h]
  lambda        <- c(lambda1DHRGSH=lambda1DHRGSH,
  ...
  theta <- c(bw=bw,
             ...
             GFR=GFR,
             CL,
             fsum=fsum,fDHRGSH=fDHRGSH,fDHRPROT=fDHRPROT,fDHRDNA=fDHRDNA,
             lambda,
             kip=kip,
             partitioning,
             Fa=Fa,
