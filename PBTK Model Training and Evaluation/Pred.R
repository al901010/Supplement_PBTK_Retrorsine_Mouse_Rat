...
PredRxODE <- function(est,
  ...
  for(row in 1:nrow(runtable)) {
    ...
    inits <- c(Alum=0,Aper=0,Aven=0,Aart=0,Aadi=0,Abon=0,Abra=0,Agut=0,Ahea=0,Akid=0,Alivvi=0,
               Alivc=0,Alun=0,Amus=0,Aski=0,Aspl=0,DHRGSH=0,DHRPROT=0,DHRDNA=0,Auri=0,Abil=0,bag=0)
    
    # Solve ODE system 
    wide <- mRetrorsine %>% rxSolve(theta,ev,inits,method="lsoda",atol=1e-12,rtol=1e-10) #,maxsteps=500000,atol=1e-12,rtol=1e-10) 
  ...
  if(add_timepoints==TRUE){
    ...
    for (row in ids) {
      ...
      if(row=="Yang_2018_GSH_mouse_liver")               {new_rows$YPRED         <- results_wide[[runid]]$GSH[is_timepoints]       }
...
PredRxODE2 <- function(est,
  ...
  for(row in 1:nrow(runtable)) {
    ...
    # Define initial conditions
    inits <- c(Alum=0,Aper=0,Aven=0,Aart=0,Aadi=0,Abon=0,Abra=0,Agut=0,Ahea=0,Akid=0,Alivvi=0,
               Alivc=0,Alun=0,Amus=0,Aski=0,Aspl=0,DHRGSH=0,DHRPROT=0,DHRDNA=0,Auri=0,Abil=0,AMetgut=0,
               AMetlivother=0)
    
    # Solve ODE system 
    wide <- mRetrorsine2 %>% rxSolve(theta,ev,inits,method="lsoda",atol=1e-12,rtol=1e-10) #,maxsteps=500000,atol=1e-12,rtol=1e-10) 
